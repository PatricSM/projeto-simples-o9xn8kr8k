...
export default function PublicSurvey() {
  ...
  const fetchCampaignData = async () => {
    try {
      ...
      if (sectionsData) {
        sectionsData.forEach((section: any) => {
          organizedSections.push({
            ...
            questions: (section.campaign_questions || []).sort((a: any, b: any) => a.order_index - b.order_index)
          });
        });
      }
      ...
      console.log('📚 Seções carregadas:', organizedSections.map(s => ({
        title: s.title,
      ...
      setResponses(allQuestions.map(q => ({
        questionId: q.id,
        value: null
      })));
  ...
  const getCurrentResponse = (questionId: string) => responses.find(r => r.questionId === questionId);
    ...
    for (const question of currentSection.questions) {
      if (question.question_type === 'multiple_choice') {
        ...
        if (conditionalLogic?.enabled && conditionalLogic?.rules?.length > 0) {
          ...
          const applicableRule = conditionalLogic.rules.find((rule: any) => 
            rule.option_value === selectedValue
          );
          ...
          if (applicableRule) {
            if (applicableRule.action === 'skip_to_end') {
              ...
              submitSurvey();
              return;
            } else if (applicableRule.action === 'go_to_section' && applicableRule.target_section_id) {
              // Ir para seção específica
              const targetSectionIndex = sections.findIndex(s => s.id === applicableRule.target_section_id);
  ...
  const submitSurvey = async () => {
    ...
    try {
      ...
      const { data: npsSuccess, error: npsError } = await supabase
        .rpc('insert_public_nps_response', {
          p_id: responseId,
      ...
      const validResponses = responses
        ...
        .map(r => ({
          ...
          question_id: r.questionId.includes('_contextual') ? r.questionId.replace('_contextual', '') : r.questionId,
          response_value: r.value,
        }));

      // Usar RPC function para inserir respostas das perguntas
      if (validResponses.length > 0) {
        const { data: questionsSuccess, error: questionsError } = await supabase
          .rpc('insert_public_question_responses', {
  ...
  const renderQuestion = (question: Question) => {
    ...
    switch (question.question_type) {
      ...
      case 'rating': {
        ...
        const isLikertMode = ratingConfig.display_mode === 'likert' && Array.isArray(ratingConfig.items);
        ...
        if (isLikertMode) {
          ...
          const likertValues: Record<string, number> = (currentResponse?.value && typeof currentResponse.value === 'object')
              ...
              <div className="space-y-4">
                ...
                {scaleLabels.some(l => l.trim()) && (
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground pt-2 border-t">
                    {scaleLabels.map((label, i) => label.trim() && (
                    ...
                    <tr>
                      ...
                      {Array.from({ length: maxRating }, (_, i) => (
                        <th key={i} className="p-2 text-center min-w-[60px]">
                          ...
                          {scaleLabels[i] && (
                            <div className="text-xs text-muted-foreground font-normal">{scaleLabels[i]}</div>
                          )}
                        </th>
                      ))}
                    </tr>
                    ...
                    >
                      {Array.from({ length: maxRating }, (_, i) => (
                        <div key={i} className="flex items-center gap-1">
                          <RadioGroupItem value={(i + 1).toString()} id={`${question.id}-m-${idx}-${i}`} />
      ...
      case 'multiple_choice': 
        ...
        if (isMultipleChoice) {
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          ...
                          updateResponse(question.id, currentValue.filter((v: string) => v !== option));
                        }
                      }}
                    />
                ...
                <div className="space-y-2 p-3 bg-gray-50 rounded-lg border">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      checked={Array.isArray(currentResponse?.value) && currentResponse.value.some((v: any) => typeof v === 'object' && v.other)}
                      onCheckedChange={(checked) => {
                        const currentValue = Array.isArray(currentResponse?.value) 
                          ? currentResponse.value 
                          : [];
                        ...
                        if (checked) {
                          updateResponse(question.id, [...currentValue.filter((v: any) => !(typeof v === 'object' && v.other)), { other: '' }]);
                        } else {
                          updateResponse(question.id, currentValue.filter((v: any) => !(typeof v === 'object' && v.other)));
                        }
                      }}
                    />
                  ...
                  {Array.isArray(currentResponse?.value) && currentResponse.value.some((v: any) => typeof v === 'object' && v.other !== undefined) && (
                    <Input
                      placeholder="Especifique..."
                      value={currentResponse.value.find((v: any) => typeof v === 'object' && v.other !== undefined)?.other || ''}
                      onChange={(e) => {
                        const currentValue = Array.isArray(currentResponse?.value) 
                          ? currentResponse.value 
                          : [];
                        
                        const otherIndex = currentValue.findIndex((v: any) => typeof v === 'object' && v.other !== undefined);
                        if (otherIndex >= 0) {
          ...
          return (
            <RadioGroup
              value={typeof currentResponse?.value === 'object' && currentResponse.value?.other !== undefined ? 'other' : (currentResponse?.value || '')}
              onValueChange={(value) => {
                if (value === 'other') {
            ...
            >
              ...
              {allowOther && (
                <div className="space-y-2 p-3 bg-gray-50 rounded-lg border">
                  ...
                  {/* Campo de texto para "Outro(a)" */}
                  {typeof currentResponse?.value === 'object' && currentResponse.value?.other !== undefined && (
                    <Input
                      placeholder="Especifique..."
        ...
        <Card className="w-full max-w-2xl">
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-4">
              {campaign.thank_you_message || 'Sua resposta foi enviada com sucesso. Agradecemos pelo seu feedback!'}
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const showBannerAsSection = showBanner && bannerConfig.position === 'first_section' && currentSectionIndex === 0;
  // Banner no topo da primeira seção
  const showBannerTopFirst = showBanner && bannerConfig.position === 'top_first_section' && currentSectionIndex === 0;
                    ...
                    <div className="mt-6 pt-4 border-t border-dashed">
                      <div className="mb-4">
                        <h4 className="text-base font-medium">
                          {replaceDynamicValues(getFollowUpQuestion(question, getCurrentResponse(question.id))?.title || '')}
                        </h4>
                      </div>
                      {getFollowUpQuestion(question, getCurrentResponse(question.id)) && 
                        renderQuestion(getFollowUpQuestion(question, getCurrentResponse(question.id))!)
                      }
                    </div>
                  )}
                </div>
