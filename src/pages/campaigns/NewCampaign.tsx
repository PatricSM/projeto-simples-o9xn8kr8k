...
const campaignSchema = z.object({
  name: z
    .string()
    .min(1, 'Nome interno é obrigatório')
    .max(200, 'Nome deve ter no máximo 200 caracteres'),
  public_title: z
    .string()
    .min(1, 'Título público é obrigatório')
    .max(200, 'Título público deve ter no máximo 200 caracteres'),
  ...
  banner_position: z
    .enum(['first_section', 'top_first_section', 'top_all_sections', 'none'])
    .optional(),
...
export default function NewCampaign() {
      ...
      if (draftCampaignId) {
        ...
        if (sections.length > 0) {
          ...
          if (sectionIdMap.size > 0) {
            setSections((prev) =>
              prev.map((s) => (sectionIdMap.has(s.id) ? { ...s, id: sectionIdMap.get(s.id)! } : s)),
            )
          }

          // Remover seções que não existem mais no state local
          const currentSectionIds = sectionsToUpsert.filter((s) => s.id).map((s) => s.id)
          const sectionsToRemove = existingSectionIds.filter(
            (id) => !currentSectionIds.includes(id),
          )

          if (sectionsToRemove.length > 0) {
            await supabase.from('campaign_sections').delete().in('id', sectionsToRemove)
          }
        }
        ...
        if (questions.length > 0) {
          ...
          if (questionIdMap.size > 0 || sectionIdMap.size > 0) {
            setQuestions((prev) =>
              prev.map((q) => {
                ...
                const newSectionId =
                  q.section_id && sectionIdMap.has(q.section_id)
          ...
          if (questionsToRemove.length > 0) {
            ...
            await supabase.from('campaign_questions').delete().in('id', questionsToRemove)
          }
        } else {
    ...
    const loadCampaignData = async () => {
      ...
      try {
        ...
        if (savedBannerConfig || campaign.banner_url) {
          setBannerConfig({
            banner_position:
              savedBannerConfig?.position || (campaign.banner_url ? 'first_section' : 'none'),
  ...
  const publishCampaign = async (data: CampaignFormData) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Erro ao publicar campanha. Tente novamente.'
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure as informações principais da campanha</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure as mensagens que aparecerão na pesquisa</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
                control={form.control}
          ...
          <Tabs defaultValue="questions" className="space-y-6">
            ...
            <TabsContent value="banner" className="space-y-6">
              <BannerUpload config={bannerConfig} onUpdate={setBannerConfig} />
            </TabsContent>
          </Tabs>
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              {draftCampaignId
                ...
                : 'Use "Salvar Rascunho" para salvar seu progresso'}
            </div>
            <div className="flex items-center gap-4">
              <Button
                type="button"
              ...
              <Button
                ...
                disabled={
                  loading ||
                  questions.length === 0 ||
                  (sections.length > 0 && questions.some((q) => !q.section_id))
                }
              >
