...
export default function BannerUpload({ config, onUpdate }: BannerUploadProps) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('campaign-banners').getPublicUrl(data.path)
            ...
            <div className="space-y-4">
              <div className="relative rounded-lg overflow-hidden border">
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  ...
                  <Button type="button" variant="destructive" size="icon" onClick={removeBanner}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              </div>
          ...
          <div>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="first_section">Primeira seção (banner como seção)</SelectItem>
                <SelectItem value="top_first_section">No topo da primeira seção</SelectItem>
                <SelectItem value="top_all_sections">No topo de todas as seções</SelectItem>
              </SelectContent>
            </Select>
          </div>
